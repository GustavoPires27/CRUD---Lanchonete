...
export default function Lanches() {
    ...
    async function consultarTodasCategorias() {
        try {
            ...
            // não interrompe as demais funções se falhar
        }
    }
    ...
    async function cadastrarLanche() {
        try {
            if (!idCategoria) {
                setResultado("Selecione uma categoria.");
                return;
            }
            ...
            const resposta = await fetch("http://localhost:3001/lanches", {
                ...
                body: JSON.stringify({ nome, descricao, preco, idCategoria })
            });
    ...
    async function alterarLanche() {
        ...
        try {
            ...
            setPreco(data.PRECO);
            // espera que a API retorne ID_CATEGORIA ou id_categoria
            setIdCategoria(data.ID_CATEGORIA || data.id_categoria || "");
                    ...
                    <tbody>
                        {lista.map(item => (
                            <tr key={item.ID_LANCHE}>
                                ...
                                <td>{item.CategoriaNome || item.NOME_CATEGORIA || item.NOME_CATEG || item.CATEGORIA || "—"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
